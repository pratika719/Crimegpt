...
import { investigationProfileService } from "@/features/case/services/investigation-profile.service";
...
async function safeInvalidate(userId: string, caseId: string) {
  try {
    ...
    logger.warn({ err }, `Failed to invalidate cache for case ${caseId}`);
  }
}
