...
import { CreateCaseMetadataInput, UpdateCaseMetadataSchema } from "@/features/case/schemas/case-metadata.schema";
  ...
  return validateActionInput(
    ...
    async (validated) => {
      ...
      try {
        ...
        logger.warn({ err }, `Failed to invalidate cache on metadata save for case ${validated.caseId}`);
      }
      ...
      return actionSuccess({
        data: toClient(metadata), // Serialize date fields for Next.js client
      });
    }
  );
}
...
export async function getCaseMetadataAction(caseId: string) {
  return validateActionInput(
    ...
    async (validatedCaseId) => {
      ...
      return actionSuccess({
        data: metadata ? toClient(metadata) : null,
      });
    }
  );
}
