import { evidenceRepository } from "@/features/case/repositories/evidence.repository";
import { createEvidenceSchema, CreateEvidenceInput, updateEvidenceSchema, UpdateEvidenceInput } from "@/features/case/schemas/evidence.schema";
...
export class EvidenceService {
  ...
  async createEvidence(caseId: string, userId: string, input: Omit<CreateEvidenceInput, "caseId">) {
    ...
    logger.info({ caseId, userId, title: parsed.title }, "[EvidenceService] Registering evidence");
  ...
  async updateEvidence(id: string, userId: string, input: UpdateEvidenceInput, caseId?: string) {
    ...
    logger.info({ id, userId }, "[EvidenceService] Updating evidence details");
  ...
  async deleteEvidence(id: string, userId: string, caseId?: string) {
    ...
    logger.info({ id, userId, title: existing.title }, "[EvidenceService] Deleting evidence record");
