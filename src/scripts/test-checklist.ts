...
async function main() {
  ...
  try {
    ...
    const testUser = await prisma.user.create({
      data: {
        email: `test-checklist-${Date.now()}@example.com`,
        name: "Test Checklist User",
      },
    });

    // 1. Create a test case dossier
    console.log("📂 Step 1: Registering case dossier...");
    const testCase = await caseService.createCase(testUser.id, {
    ...
    const item1 = await checklistService.createChecklistItem(testCase.id, testUser.id, {
      title: "Aquire CCTV footage of scene",
    });
    console.log(`   Item 1 created: "${item1.title}" (ID: ${item1.id})`);

    const item2 = await checklistService.createChecklistItem(testCase.id, testUser.id, {
    ...
    const item1Completed = await checklistService.updateChecklistItem(item1.id, testUser.id, {
      completed: true,
    });
    ...
    const activities = await caseActivityRepository.findByCaseId(testCase.id, testUser.id);
    const completedActivity = activities.find((act: any) => act.activityType === "CHECKLIST_ITEM_COMPLETED");
    ...
    const item1Incomplete = await checklistService.updateChecklistItem(item1.id, testUser.id, {
      completed: false,
    });
    ...
    await checklistService.deleteChecklistItem(item2.id, testUser.id);
    console.log("   Item 2 deleted.");

    // Verify item2 is deleted
    const items = await checklistService.getChecklistByCaseId(testCase.id, testUser.id);
    if (items.some((item: any) => item.id === item2.id)) {
    ...
    // 7. Clean up
    console.log("📂 Step 7: Purging test user dossier...");
