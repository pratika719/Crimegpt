      ...
      <div className="flex items-center justify-between border-b border-zinc-200 dark:border-zinc-800 pb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-[9px] font-mono font-bold border ${
            error
              ? "bg-rose-50 text-rose-700 dark:bg-rose-950/20 dark:text-rose-400 border-rose-200/30"
              : diagnostics 
            ...
            {isPending ? "ANALYZING..." : error ? "ENGINE ERROR" : diagnostics ? "INSIGHTS COMPILED" : "AI STANDBY"}
          </span>
        </div>
      </div>
      ...
      <div className="relative rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 shadow-sm overflow-hidden p-6 md:p-8 space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-zinc-100 dark:border-zinc-800/60 pb-5">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`text-[10px] font-mono font-bold uppercase bg-zinc-100 dark:bg-zinc-800 px-2 py-0.5 rounded ${
              error ? "text-rose-500" : diagnostics ? "text-emerald-600" : "text-zinc-500"
            }`}>
              {isPending ? "Generating Metrics" : error ? "Error Encountered" : diagnostics ? "Dashboard Active" : "Awaiting Data Trigger"}
            </span>
          </div>
        </div>
        ...
        {diagnostics ? (
          ...
          <div className="grid gap-4 md:grid-cols-3 relative">
            ...
            {/* Premium Lock / Activation / Error Overlay */}
            {error ? (
              <div className="absolute inset-0 flex items-center justify-center bg-white/40 dark:bg-zinc-900/40 backdrop-blur-[2px] rounded-xl border border-rose-200/50 dark:border-rose-900/50 p-4">
                <div className="bg-white dark:bg-zinc-950 border border-rose-200 dark:border-rose-900/60 rounded-xl px-6 py-7 text-center shadow-xl max-w-md space-y-4">
                  <div className="mx-auto h-12 w-12 rounded-full bg-rose-50 dark:bg-rose-950/40 flex items-center justify-center border border-rose-200 dark:border-rose-900/50 shadow-inner text-rose-600 dark:text-rose-400">
                    <AlertCircle className="h-6 w-6" />
                  </div>
                  
                  <div className="space-y-1.5">
                    <h5 className="text-sm font-bold text-zinc-900 dark:text-zinc-100 font-sans tracking-tight">
                      Diagnostics Engine Error
                    </h5>
                    <p className="text-xs text-rose-600 dark:text-rose-400 font-medium bg-rose-50/80 dark:bg-rose-950/30 p-3 rounded-lg border border-rose-100 dark:border-rose-900/30 leading-relaxed text-left">
                      {error}
                    </p>
                  </div>
                  ...
                  <div className="pt-1">
                    <button
                      onClick={handleRunDiagnostics}
                      disabled={isPending}
                      className="w-full inline-flex items-center justify-center gap-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold py-2.5 rounded-lg transition-colors shadow-sm cursor-pointer disabled:opacity-50"
                    >
